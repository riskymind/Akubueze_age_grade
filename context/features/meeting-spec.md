...
model Meeting {
  ...
  minutesUrl  String?       // UploadThing ufsUrl
  notes       String?
  createdAt   DateTime      @default(now())
  updatedAt   DateTime      @updatedAt
...
model Payment {
  ...
  amount      Float
  description String?
  receiptUrl  String?       // UploadThing ufsUrl
  paidAt      DateTime?
  dueDate     DateTime?
  createdAt   DateTime      @default(now())
  updatedAt   DateTime      @updatedAt
...
> - `Payment.receiptUrl` stores an UploadThing `ufsUrl` for receipt files.
...
│   ├── uploadthing.ts            # Generated UploadThing helpers
│   ├── resend.ts
│   └── utils.ts
│
├── prisma/
...
Meeting minutes and receipts are user-uploaded files (PDFs). UploadThing integrates natively with Next.js App Router — uploads flow through your own API route handler, require no public credentials in the browser, and return typed `ufsUrl` values. Setup is a single `UPLOADTHING_TOKEN` env var with no separate S3 or CDN configuration.
